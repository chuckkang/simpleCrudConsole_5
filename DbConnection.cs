...
namespace DbConnection
{
	public class DbConnector
	{
		...
		static string db = "fuelcenter"; //Change to your schema name
		...
		internal static IDbConnection Connection
		{
			get
			{
				return new MySqlConnection($"Server={server};Port={port};Database={db};UserID={user};Password={pass};SslMode=None;character set=utf8;");
			}
		}
				...
				using (IDbCommand command = dbConnection.CreateCommand())
				{
					...
					using (IDataReader rdr = command.ExecuteReader())
					{
						while (rdr.Read())
						{
							...
							{
								dict.Add(rdr.GetName(i), rdr.GetValue(i).ToString());
							}
							result.Add(dict);
						}
						return result;
